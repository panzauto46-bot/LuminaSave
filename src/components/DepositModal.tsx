...
export default function DepositModal() {
  ...
  const { network, tokenDecimals, tokenSymbol, isSelectedVaultSupportedOnChain } =
    useYoRuntime(selectedVault);

  const usdcAddress = VAULTS[selectedVault]?.underlying?.address[chainId as keyof typeof VAULTS[typeof selectedVault]['underlying']['address']];
      ...
      dispatch({
        ...
        payload: {
          ...
          patch: {
            ...
            message: approveHash
              ? `Approve + deposit sent on ${network}. Tx ${shortHash(h)}`
              : `Deposit sent on ${network}. Tx ${shortHash(h)}`,
            type: 'pending',
          },
        },
      });
      ...
      dispatch({
        ...
        payload: {
          ...
          patch: {
            ...
            message: `$${amountNum.toFixed(2)} deposited to ${activeGoal?.name ?? 'goal'} on ${network}.`,
            type: 'success',
          },
        },
      });
  ...
  // Sync SDK step → UI step
  useEffect(() => {
    if (step === 'approving' || step === 'depositing' || step === 'waiting' || step === 'switching-chain') {
      setUiStep('loading');
    }
  }, [step]);
  ...
  const handleConfirm = async () => {
    ...
    if (!isSelectedVaultSupportedOnChain || !usdcAddress || !network) {
      setUiStep('failed');
    ...
    try {
      await deposit({ token: usdcAddress, amount: amountAtomic, chainId });
    } catch (err) {
  ...
  const stepLabel = step === 'approving' ? 'Approving token...' : step === 'depositing' ? 'Depositing...' : step === 'waiting' ? 'Waiting for confirmation...' : step === 'switching-chain' ? 'Switching chain...' : 'Processing...';
            ...
            <>
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  className={`w-full px-4 py-3 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 ${dark
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-8 pr-4 py-3 rounded-xl text-lg font-bold focus:outline-none focus:ring-2 ${dark
                ...
                <div className="flex gap-2 mt-2">
                  {[50, 100, 250, 500].map((v) => (
                    <button
                      ...
                      className={`flex-1 py-1.5 rounded-lg text-xs font-semibold ${dark ? 'bg-white/5 hover:bg-white/10 text-gray-300' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'
                        }`}
                    >
                      ${v}
                    </button>
              ...
              {amountNum > 0 && (
                <motion.div
                  ...
                  className={`rounded-xl p-4 mb-4 space-y-2 ${dark ? 'bg-white/5 border border-white/10' : 'bg-gray-50 border border-gray-200'
                    }`}
                >
              ...
              <button
                ...
                className={`w-full py-4 rounded-2xl font-bold text-base flex items-center justify-center gap-2 transition-all ${amountNum > 0
                  ? dark
